		  ...
		  if(it->second.aliveByte==0 || 
		    ...
		    if(cleanSegment==THREADSHOLD){
		       for(auto segmentIt:reUsedTable){
		       	   if(segmentIt.second.alive) continue;
	 ...
	 //   struct SegmentSummary *summary=(struct SegmentSummary*)malloc(sizeof(struct SegmentSummary));
		...
		int tmp=length;
		 if(tmp>0){
					...
					memcpy(segmentCache->dataB[blockIndex].data,buffer,BLOCK_SIZE-100);
      ...
      if(segmentCache->summary->segmentNo==logAddress1.segmentNo){    // check current segment cache
      	 ...
      	 //   	 printf("test blockNo %d i=%d\n ",segmentCache->dataB[i].blockNo,i);
      		...
      		// 	printf("i= %d  content =%s \n", i,segmentCache->dataB[i].data);
      		 	...
      		 	memcpy(buffer,&segmentCache->dataB[i].data,BLOCK_SIZE-100);
      		 }
      	    return 0;
      }
      int index=0;
      
      for(index=N-1;index>=0;index--){  // start from end of queue search (most laster on the end)
      		    ...
      		    int i;
      		 	...
      		 	//	cout<<p.dataB[i+1].data<<"  "<<index<<endl;
      		 		...
      		 		//	 memcpy(buffer,block->second.data,BLOCK_SIZE);
      		 			 memcpy(buffer,block,BLOCK_SIZE-100);
      		 		}
      		 	}else{
	      	   	 	 ...
	      	   	 	 //  	 printf(" read from flash i=%d blockNo=%d \n",i,pdata[i].blockNo);	
      	   				...
      	   				if(i<=BLOCK_NUMBER-1){
			      	   		...
			      	   		//	printf("i=%d   content  %s\n",i,pdata[i].data );
		      					...
		      					memcpy(buffer,pdata[i].data,BLOCK_SIZE-100);
		      		 		 }
	      		 	    }else{
